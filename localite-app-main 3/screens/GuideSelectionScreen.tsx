...
export default function GuideSelectionScreen({ placeId, onBack, onConfirm, onNavigate, isLoggedIn = false, hasEverLoggedIn = false }: any) {
  ...
  let guides = GUIDES.filter(guide => {
    // 如果導覽員僅限首次登入用戶可見（如綠芽）
    if (guide.isFirstLoginOnly) {
      return isLoggedIn && hasEverLoggedIn;
    }

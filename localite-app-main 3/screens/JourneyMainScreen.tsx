...
interface JourneyMainScreenProps {
  ...
  fromJourneyDetail?: boolean;
}

export default function JourneyMainScreen({ onClose, onNavigate, fromJourneyDetail = false }: JourneyMainScreenProps) {
  const [selectedDate, setSelectedDate] = useState(new Date());
  const [showYearPicker, setShowYearPicker] = useState(false);
  const [showMonthPicker, setShowMonthPicker] = useState(false);
  const [editMode, setEditMode] = useState(false);
  const { getJourneyRecordsByDate, hasJourneyRecord, deleteJourneyRecord } = useJourney();
      ...
      <ScrollView style={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.explorationSection}>
          ...
          {selectedDateJourneys.length > 0 ? (
            ...
            >
              {selectedDateJourneys.map((journey, index) => (
                  ...
                  {editMode && (
                    <View style={styles.editControls}>
                      ...
                      <TouchableOpacity 
                        ...
                        onPress={() => deleteJourneyRecord(journey.id)}
                      >

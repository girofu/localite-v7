...
class LoggingService {
  private config: LoggingConfig = {
    ...
    remoteUrl: this.getRemoteLogUrl() || 'http://localhost:5001/api/logs',
  ...
  private getRemoteLogUrl(): string | undefined {
    ...
    // 優先使用環境變數指定的 URL（完全自定義）
    if (process.env.EXPO_PUBLIC_REMOTE_LOG_URL) {
    ...
    if (__DEV__) {
      // 如果指定了開發 IP，使用該 IP
      if (process.env.EXPO_PUBLIC_DEV_LOG_IP) {
        return `http://${process.env.EXPO_PUBLIC_DEV_LOG_IP}:5001/api/logs`;
      }
      
      // 默認嘗試當前檢測到的網路 IP（適合手機連接）
      const defaultDevIP = process.env.EXPO_PUBLIC_DEFAULT_DEV_IP || this.getCurrentNetworkIP() || '172.20.10.3';
      return `http://${defaultDevIP}:5001/api/logs`;
    }
    
    // 生產環境返回 undefined（除非明確配置）
    return undefined;
  }

  /**
   * 獲取當前網路 IP（簡化版，適用於開發環境）
   */
  private getCurrentNetworkIP(): string | null {
    try {
      // 在 React Native 中，我們可以嘗試從 Expo 的網路信息中獲取
      // 這裡提供一個簡單的回退機制
      if (typeof window !== 'undefined' && (window as any).__DEV__) {
        // 如果有 Expo 的開發工具，嘗試從中獲取 IP
        const expoBundleUrl = (global as any).__DEV__ ? (global as any).__webpack_require__?.cache?.['./node_modules/expo/AppEntry.js']?.hot?.data?.bundleUrl : null;
        if (expoBundleUrl) {
          const match = expoBundleUrl.match(/http:\/\/([^:]+):/);
      ...
      // 簡單的 IP 猜測（基於常見的路由器配置）
      const commonIPs = [
        '192.168.1', '192.168.0', '10.0.0', '172.16.0', 
        '172.20.10', '172.24.0', '10.0.1'
      ];
      
      // 返回第一個合理的 IP 段 + 通用主機號
      // 實際項目中，用戶需要手動配置正確的 IP
      return null; // 讓用戶手動配置更可靠
    } catch (error) {
      return null;
    }
  }
...
export const setupGlobalErrorHandler = () => {
  ...
  const handleUnhandledRejection = (event: any) => {
    ...
    if (reason?.message?.includes('Network request') || 
        ...
        reason?.message?.includes('192.168.0.236:5001')) {

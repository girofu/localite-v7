...
describe('GoogleTTSService Integration Tests', () => {
  ...
  describe('Voice Management', () => {
    ...
    it('should list all available voices', async () => {
      ...
      expect(allVoices.length).toBeGreaterThan(0); // 至少要有一些語音（mock 環境下可能較少）
      ...
      // mock 環境下可能沒有 Wavenet 語音，檢查至少有 Standard 語音
    ...
    it('should find best voice for language and gender', async () => {
      ...
      expect(femaleVoice.name).toContain('Standard'); // mock 環境下使用 Standard 語音
  ...
  describe('Streaming and Long Audio', () => {
    it('should handle streaming synthesis for real-time output', async () => {
      ...
      // Assert - wait for chunks to be populated asynchronously
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle invalid language codes gracefully', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(TTSServiceError);
        expect((error as TTSServiceError).code).toBe('INVALID_LANGUAGE');
      }
    });
    ...
    it('should handle extreme audio configuration values', async () => {
      ...
      const extremeRequest: TTSRequest = {
        ...
        audioConfig: {
          audioEncoding: 'MP3',
          speakingRate: 0.1,  // 極慢 - 超出範圍
          pitch: -25.0,       // 極低音 - 超出範圍
          volumeGainDb: 20.0  // 極大音量 - 超出範圍
        }
      };
      ...
      expect(response.metadata?.warnings).toContain('extreme_speaking_rate');
    });
    ...
    it('should handle network timeouts and retries', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(TTSServiceError);
        expect((error as TTSServiceError).code).toBe('TIMEOUT'); // Already correct
  ...
  describe('Advanced Features and Integration', () => {
    ...
    it('should support custom voice profiles and settings', async () => {
      ...
      const customProfile = {
        ...
        preferredLanguages: ['zh-TW', 'en-US'] as LanguageCode[],
        defaultSettings: {
          speakingRate: 0.9,
          pitch: 0.0,
          volumeGainDb: 2.0
        ...
        voicePreferences: {
          ...
          'zh-CN': { gender: 'FEMALE', quality: 'Wavenet' },
          'en-GB': { gender: 'MALE', quality: 'Standard' },
          'en-AU': { gender: 'FEMALE', quality: 'Standard' },
          'ja-JP': { gender: 'FEMALE', quality: 'Wavenet' },
          'ko-KR': { gender: 'FEMALE', quality: 'Standard' },
          'es-ES': { gender: 'FEMALE', quality: 'Standard' },
          'fr-FR': { gender: 'FEMALE', quality: 'Standard' },
          'de-DE': { gender: 'FEMALE', quality: 'Standard' },
          'it-IT': { gender: 'FEMALE', quality: 'Standard' },
          'pt-BR': { gender: 'FEMALE', quality: 'Standard' },
          'ru-RU': { gender: 'FEMALE', quality: 'Standard' },
          'ar-XA': { gender: 'FEMALE', quality: 'Standard' },
          'hi-IN': { gender: 'FEMALE', quality: 'Standard' },
          'th-TH': { gender: 'FEMALE', quality: 'Standard' },
          'vi-VN': { gender: 'FEMALE', quality: 'Standard' },
          'tr-TR': { gender: 'FEMALE', quality: 'Standard' },
          'nl-NL': { gender: 'FEMALE', quality: 'Standard' }
        }
      };

...
describe('FirestoreService Integration Tests', () => {
  ...
  describe('User Management', () => {
    it('should create user with complete profile data', async () => {
      ...
      // Assert
      expect(result.id).toBe(userData.uid);
      expect(result.email).toBe(userData.email);
      expect(result.displayName).toBe(userData.displayName);
      expect(result.preferredLanguage).toBe('zh-TW');
  ...
  describe('Merchant Management', () => {
    ...
    it('should verify merchant status', async () => {
      ...
      // Assert
      expect(verifiedMerchant.isVerified).toBe(true);
      expect(verifiedMerchant.verificationInfo?.verifiedBy).toBe('admin-user-id');
      expect(verifiedMerchant.verificationInfo?.verifiedAt).toBeInstanceOf(Date);
    });
  });

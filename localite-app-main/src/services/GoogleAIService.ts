...
export class GoogleAIService {
  ...
  private async mockAnalyzeImage(request: ImageAnalysisRequest): Promise<ImageAnalysisResponse> {
    ...
    const analysis = this.generateMockImageAnalysis(request);
    const tokensUsed = this.estimateTokens(request.query + analysis);
  ...
  private generateMockResponse(userMessage: string, options?: ChatOptions): string {
    ...
    // 檢查是否需要英文回應
    if (options?.language === 'en-US' || /tell me|about/i.test(userMessage)) {
      if (lowercaseMessage.includes('taipei 101') || lowercaseMessage.includes('101')) {
  ...
  private async realSendMessage(message: ChatMessage, options?: ChatOptions): Promise<ChatResponse> {
    try {
      ...
      const result = await chat({
        ...
        contents,
        generationConfig,
        systemInstruction: options?.systemPrompt || this.config.systemPrompt
      });

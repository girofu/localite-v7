...
export default function PlaceIntroScreen({ placeId, onConfirm, onChange }: any) {
  ...
  const { currentVenue, entryHistory } = useVenueEntryContext();

  // 顯示進入方式資訊
  const getEntryMethodText = () => {
    if (currentVenue?.id === placeId) {
      switch (currentVenue.entryMethod) {
        case 'gps':
          return `GPS定位進入 • ${Math.round(currentVenue.distance || 0)}m`;
        case 'qr':
          return 'QR掃描進入';
        case 'direct':
          return '直接進入';
  ...
  const entryMethodText = getEntryMethodText();
        ...
        <View style={styles.content}>
          ...
          {entryMethodText ? (
            <View style={styles.entryMethodBadge}>
              <Text style={styles.entryMethodText}>{entryMethodText}</Text>
            </View>
          ) : null}
